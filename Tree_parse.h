...
public: 
	...
	Node* create_new_node(string rule) {
	...
	bool check_all_terminals(vector<Node*> &n) {
	...
	//проверить а корень подходить дл€ ребенка;
		...
		if (root->info == "Procedure") {
			int n;
		}
		for (int i = rules[ind_in_rules].second.size() - 1; i >= 0; i--) {
			Node* new_child = new Node(rules[ind_in_rules].second[i]);
	...
	void print_Tree(Node* tree_node, int depth = 0) {
